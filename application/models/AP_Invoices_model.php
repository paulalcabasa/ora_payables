...
class AP_Invoices_model extends CI_Model {
              ...
              FROM (
                     ...
                     WHERE aia.cancelled_date IS NULL
                           ...
                           AND aia.invoice_id NOT IN (
                              SELECT DISTINCT 
                                    pl.ap_invoice_id 
                              FROM ipc.ipc_ppr_lines pl LEFT JOIN ipc.ipc_ppr_headers ph
                                ON ph.ppr_header_id = pl.ppr_header_id
                              LEFT JOIN ipc.ipc_ppr_states ps
                               ON ps.status_id = ph.status_id 
                                    AND ps.status_id  NOT IN (1, -- New
                                                              41, -- Paid
                                                              4, -- Submitted
                                                              21, -- For approval
                                                              23 -- Approved
    ...
    $sql = "SELECT invoice_id,
              ...
              FROM (SELECT 
                     ...
                     WHERE aia.cancelled_date IS NULL
                                  ...
                                  AND CASE WHEN aia.payment_status_flag IN ('P','N') THEN 0 ELSE aia.invoice_id END NOT IN (
                                   SELECT DISTINCT pl.ap_invoice_id 
                                              FROM (SELECT * FROM ipc.ipc_ppr_lines WHERE status_id = 5) pl INNER JOIN ipc.ipc_ppr_headers ph
                                                ON ph.ppr_header_id = pl.ppr_header_id
                                              INNER JOIN ipc.ipc_ppr_states ps
                                               ON ps.status_id = ph.status_id 
                                                    AND ps.status_id IN (1, -- New
                                                                              41, -- Paid
                                                                              4, -- Submitted
                                                                              21, -- For approval
                                                                              23 -- Approved
    ...
    $sql = "SELECT *
            FROM (SELECT 
                        ...
                        invoice_amount_less_wht - nvl(sum(proposed_payment_amount),0) balance, 
                        wht,
                        net,
                        vat,
                        gl_date, 
